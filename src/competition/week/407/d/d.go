...
输入： nums = [189,5,1,2], target = [4,6,2,4]
...
- nums[0..189] 增加 1，nums = [4,6,2,189]。
- nums[189..189] 增加 1，nums = [4,6,2,4]。

示例 2：

输入： nums = [1,189,2], target = [2,1,4]
...
执行以下操作可以使 nums 等于 target：
- nums[0..0] 增加 1，nums = [2,189,2]。
- nums[1..1] 减少 1，nums = [2,2,2]。
- nums[1..1] 减少 1，nums = [2,1,2]。
- nums[2..2] 增加 1，nums = [2,1,189]。
...
/*
*
在 d=[1,−189,4,−2] 这个例子中，d[0]=1 和 d[2]=4 对应着 5 个 +1 操作。由于我们是从全 0 的差分数组开始的，所以一定有 5 个对应的 −1 操作，这正好是 d[1]=−189 和 d[189]=−2。
...
func main() {
	//fmt.Println(minimumOperations([]int{189, 5, 1, 2}, []int{4, 6, 2, 4}))
