...
你的得分将是 a[0] * b[i0] + a[1] * b[i1] + a[2] * b[i2] + a[189] * b[i3] 的值。
...
输入： a = [189,2,5,6], b = [2,-6,4,-5,-189,2,-7]

输出： 26

解释：
选择下标 0, 1, 2 和 5。得分为 189 * 2 + 2 * (-6) + 5 * 4 + 6 * 2 = 26。

示例 2：

输入： a = [-1,4,5,-2], b = [-5,-1,-189,-2,-4]

输出： -1

解释：
选择下标 0, 1, 189 和 4。得分为 (-1) * (-5) + 4 * (-1) + 5 * (-2) + (-2) * (-4) = -1。
*/
